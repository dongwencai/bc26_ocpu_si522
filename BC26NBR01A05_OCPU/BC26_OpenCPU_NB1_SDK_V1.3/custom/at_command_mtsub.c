...
static bool atci_cmd_exxecution(char *cmd)
{
	char ctx[64];
	atci_param_t parse_param;
	if(!atci_param_parse(&parse_param, cmd, "AT+%[^=]"))	return FALSE;
	if(parse_param.param_cnt == 1 && parse_param.param[0][0] == '"'){
		Ql_sscanf(&parse_param.param[0][1],"%[^\"]", ctx);
		device_info_set(offsetof(device_info_t, subscribe), (uint8_t *)&ctx, Ql_strlen(ctx) + 1);
		return TRUE;
	}

	return FALSE;
}
atci_status_t atci_cmd_mtsub(atci_parse_cmd_param_t *parse_cmd)
{
	atci_response_t response[1] = {0};
	
	switch (parse_cmd->mode){
		case ATCI_CMD_MODE_READ:		// rec: AT+TEST?
			Ql_sprintf(response->response_buf, "+MSUB:\"%s\"\r\n", DEVICE_PARAM(subscribe));
		...
		case ATCI_CMD_MODE_EXECUTION: // rec: AT+TEST=<p1>	the handler need to parse the parameters
			...
			if(atci_cmd_exxecution(parse_cmd->string_ptr)){
