...
static bool atci_cmd_execution(char *cmd)
{
	...
	if(parse_param.param_cnt == 1 && parse_param.param[0][0] == '"') {
		...
		device_info_set(offsetof(device_info_t, log_item), (uint8_t *)ctx, Ql_strlen(ctx) + 1);
		return TRUE;
	}
	return FALSE;
}

atci_status_t atci_cmd_ttdebug(atci_parse_cmd_param_t *parse_cmd)
{
	atci_response_t response[1] = {0};
	switch (parse_cmd->mode){
		case ATCI_CMD_MODE_READ:		// rec: AT+TEST?
			Ql_sprintf(response->response_buf, "+TTDEBUG: \"%s\"\r\n", DEVICE_PARAM(log_item));
