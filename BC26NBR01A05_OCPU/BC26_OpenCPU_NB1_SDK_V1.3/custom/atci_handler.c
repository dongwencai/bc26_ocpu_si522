...
atci_status_t atci_input_command_handler(atci_send_input_cmd_msg_t* cmd_msg)
{
  ...
  parse_cmd->string_len = Ql_strlen(cmd_msg->input_buf);
	...
	for(i = 0;i < g_atci_registered_table_number; i++) {
		...
		for(j = 0;j < item_table_size;j++) {
			...
			if (parse_cmd->hash_value1 == handler_item->hash_value1 && parse_cmd->hash_value2 == handler_item->hash_value2 &&
			  !Ql_strcmp(parse_cmd->command_head, handler_item->command_head)) {
				ret = ATCI_STATUS_OK;
				break;
			}
			handler_item = NULL;
	...
	for (i = 0; i < hdlr_number; i++) {
		for (j = 0; j < i; j++) {
			if ((parse_cmd + i)->hash_value1 == (parse_cmd + j)->hash_value1 && (parse_cmd + i)->hash_value2 == (parse_cmd + j)->hash_value2 &&
				  !Ql_strcmp((parse_cmd + i)->command_head, (parse_cmd + j)->command_head)) {
...
static atci_status_t atci_local_dispatch_cm4_general_handler(atci_parse_cmd_param_ex_t *parse_cmd)
{
  ...
  for(i = 0;i < g_atci_registered_table_number;i++) {
    ...
    for(j = 0;j < item_table_size;j++) {
      handler_item = &(g_atci_cm4_general_hdlr_tables[i].item_table[j]);
      if (parse_cmd->hash_value1 == handler_item->hash_value1 && parse_cmd->hash_value2 == handler_item->hash_value2 &&
    	  !Ql_strncmp((char *)parse_cmd->string_ptr, handler_item->command_head, parse_cmd->name_len)) {
        ret = ATCI_STATUS_OK;
        break;
      }
      handler_item = NULL;
  ...
  if ((NULL != handler_item) && (ret == ATCI_STATUS_OK)) {
		if(parse_cmd->mode == ATCI_CMD_MODE_TESTING){
			if(handler_item->command_usage){
				atci_response_t response;
				Ql_memset(&response, 0x00, sizeof(response));
				response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;
				response.response_len = Ql_strlen(handler_item->command_usage);
				Ql_strncpy(response.response_buf, handler_item->command_usage, ATCI_UART_TX_FIFO_BUFFER_SIZE);
				atci_send_response(&response);
				return ret;
			}
		}
