...
void RIL_SOC_QMTSUB_SET_RES(s32 result)
{
	sub_result = result;
	Ql_OS_SetEvent(sys_event_id, EVENT_MQTT_SUB_RESP);
}
...
s32 RIL_SOC_WaitEvent(u32 evtId, u32 evtFlag,u32 block_time)
{
	u32 *pevent = (u32 *)evtId;
	u16 timeout = block_time / 100;
	while(--timeout){
		if(*pevent & evtFlag){
			*pevent &= ~evtFlag;
			return evtFlag;
...
s32 RIL_SOC_QMTOPEN(char *host, u16 port)
{
  ...
  if (RIL_AT_SUCCESS != ret){
	...
	event = Ql_OS_WaitEvent(sys_event_id, EVENT_MQTT_OPEN_RESP, 30000);
...
s32 RIL_SOC_QMTCONN(char *clientid, char *user, char *passwd)
{
	...
	event = Ql_OS_WaitEvent(sys_event_id, EVENT_MQTT_CONN_RESP, 30000);
...
s32 RIL_SOC_QMTPUB(char *topic, u8 qos, u16 msgid, char *text)
{
	...
	event = Ql_OS_WaitEvent(sys_event_id, EVENT_MQTT_PUB_RESP, 11000);
...
s32 RIL_SOC_QMTSUB(char *topic)
{
	...
	event = Ql_OS_WaitEvent(sys_event_id, EVENT_MQTT_SUB_RESP, 11000);
